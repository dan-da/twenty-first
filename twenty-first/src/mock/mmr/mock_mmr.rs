...
///
/// It implements an in-memory Mmr using a Vec<Digest> as the data store.
///
/// It is not intended or tested for any kind of production usage.
///
    ...
    fn mmr_append_test() {
        ...
        for size in 1..260 {
            ...
            for (leaf_index, leaf_hash) in leaf_hashes_blake3.clone().into_iter().enumerate() {
                ...
                // Verify that membership proofs are the same as generating them from an MockMmr
