...
mod mmr_membership_proof_test {
    ...
    fn get_node_indices_simple_test() {
        ...
        let archival_mmr: StorageMmr<H, OrdinaryVec<Digest>> =
    ...
    fn get_peak_index_simple_test() {
        ...
        let mut archival_mmr: StorageMmr<H, OrdinaryVec<Digest>> =
    ...
    fn update_membership_proof_from_leaf_mutation_test() {
        ...
        let mut an_archival_mmr: StorageMmr<H, OrdinaryVec<Digest>> =
            get_rustyleveldb_ammr_from_digests(leaf_hashes.clone());
        let original_archival_mmr: StorageMmr<H, OrdinaryVec<Digest>> =
            get_rustyleveldb_ammr_from_digests(leaf_hashes.clone());
    ...
    fn update_membership_proof_from_leaf_mutation_blake3_big_test() {
        ...
        for leaf_count in 0..=22 {
            ...
            for i in 0..leaf_count {
                ...
                let mut modified_archival_mmr: StorageMmr<H, OrdinaryVec<Digest>> =
                    get_rustyleveldb_ammr_from_digests(leaf_hashes.clone());
    ...
    fn update_membership_proof_from_append_simple() {
        ...
        for i in 0..leaf_count {
            ...
            let (mut membership_proof, old_peaks): (MmrMembershipProof<H>, Vec<Digest>) =
                archival_mmr.prove_membership(leaf_index);
            let mut appended_archival_mmr: StorageMmr<H, OrdinaryVec<Digest>> =
                get_rustyleveldb_ammr_from_digests(leaf_hashes.clone());
    ...
    fn update_membership_proof_from_append_big_tests() {
        ...
        for leaf_count in 0..68u64 {
            // 1. Build an StorageMmr with a variable amount of leaves
            ...
            for leaf_index in 0..leaf_count {
                // 2. Create an equivalent StorageMmr, but with `new_leaf_digest` appended
                let mut appended_archival_mmr: StorageMmr<H, OrdinaryVec<Digest>> =
                    get_rustyleveldb_ammr_from_digests(leaf_digests.clone());
                appended_archival_mmr.append(new_leaf_digest);
                let new_peaks = appended_archival_mmr.get_peaks();

                // 3. Create membership proof for StorageMmr from before `new_leaf_digest`
                ...
                // 6. Assert that updating an old membership proof is equivalent to getting a proof for an appended StorageMmr
            ...
            // ...and then create an equivalent StorageMmr, but with `new_leaf_digest` appended...
            let mut appended_archival_mmr: StorageMmr<H, OrdinaryVec<Digest>> =
                get_rustyleveldb_ammr_from_digests(leaf_digests.clone());
            appended_archival_mmr.append(new_leaf_digest);
            let new_peaks = appended_archival_mmr.get_peaks();
